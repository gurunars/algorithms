...
class SympyNode(nodes.General, nodes.Element):
    pass
...
class LaconicPrinter(LatexPrinter):
    ...
    def _print_Lambdified(self, expr: Any):
        return self.doprint(expr.printed)
...
def visit_sympy_node_html(translator: SphinxTranslator, node: SympyNode) -> None:
...
class Sympy(SphinxDirective):
    ...
    def run(self) -> List[SympyNode]:
        ...
        return [SympyNode(expressions = [get_mathjax(it) for it in expressions])]
...
def setup(app: Sphinx):
    ...
    app.add_directive("sympy-output", Sympy)
    return {
        'version': '0.1',
